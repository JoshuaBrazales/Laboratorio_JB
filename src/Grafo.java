...
public class Grafo {
    private ArrayList<modelo.Vertice> vertices = new ArrayList<>();
    private int[][] matrizAdyacencia = new int[50][50];
    public void agregarVertice(String id, int x, int y) {
        vertices.add(new modelo.Vertice(id, x, y));
    }
    public void agregarArista(String id1, String id2) {
        modelo.Vertice v1 = buscar(id1);
        modelo.Vertice v2 = buscar(id2);
        if (v1 != null && v2 != null) {
    ...
    private modelo.Vertice buscar(String id) {
        for (modelo.Vertice v : vertices) if (v.getId().equals(id)) return v;
        return null;
    }
    public ArrayList<modelo.Vertice> getVertices() { return vertices; }
